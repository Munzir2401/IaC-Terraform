...
resource "aws_lb_target_group" "tg" {
  ...
  health_check {
    path                = replace(each.value, "*", "/") # Use a valid path like /cars/ for the health check
    interval            = 30
    timeout             = 5
    healthy_threshold   = 2
    unhealthy_threshold = 2
...
resource "aws_cloudwatch_metric_alarm" "unhealthy_hosts" {
  ...
  statistic           = "Average"
  threshold           = "1"
  alarm_description   = "Alarm when there is one or more unhealthy hosts in the default target group."
  alarm_actions       = [aws_sns_topic.alarms.arn]
  ok_actions          = [aws_sns_topic.alarms.arn]

  dimensions = {
    LoadBalancer = aws_lb.alb.name
    TargetGroup  = aws_lb_target_group.default_tg.name
  }
}

resource "aws_cloudwatch_metric_alarm" "path_based_unhealthy_hosts" {
  for_each            = var.context_paths
  alarm_name          = "${var.alb_name}-${each.key}-unhealthy-hosts"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
  metric_name         = "UnHealthyHostCount"
  namespace           = "AWS/ApplicationELB"
  ...
  statistic           = "Average"
  threshold           = "1"
  alarm_description   = "Alarm when there is one or more unhealthy hosts in the ${each.key} target group."
  alarm_actions       = [aws_sns_topic.alarms.arn]
  ok_actions          = [aws_sns_topic.alarms.arn]

  dimensions = {
    LoadBalancer = aws_lb.alb.name
    TargetGroup  = aws_lb_target_group.tg[each.key].name
  }
}

resource "aws_cloudwatch_metric_alarm" "homepage_high_cpu" {
  alarm_name          = "${var.alb_name}-homepage-high-cpu"
  ...
  statistic           = "Average"
  threshold           = "50"
  alarm_description   = "Alarm when CPU exceeds 50% for the homepage"
  alarm_actions       = [aws_sns_topic.alarms.arn]
  ok_actions          = [aws_sns_topic.alarms.arn]

  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.homepage_asg.name
  }
}

resource "aws_cloudwatch_metric_alarm" "path_based_high_cpu" {
  for_each            = var.context_paths
  alarm_name          = "${var.alb_name}-${each.key}-high-cpu"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "2"
  metric_name         = "CPUUtilization"
  namespace           = "AWS/EC2"
  period              = "60"
  statistic           = "Average"
  threshold           = each.key == "cars" ? 40 : 50
  alarm_description   = "Alarm when CPU exceeds ${each.key == "cars" ? 40 : 50}% for the ${each.key} target group"
  alarm_actions       = [aws_sns_topic.alarms.arn]
  ok_actions          = [aws_sns_topic.alarms.arn]

  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.per_tg_asg[each.key].name
  }
}
